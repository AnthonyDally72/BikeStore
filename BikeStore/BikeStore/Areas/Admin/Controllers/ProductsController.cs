...
namespace BikeStore.Controllers
{
    ...
    {
        private readonly ApplicationDbContext _db; // utilizes dependency injection to input data from the sqldatabase
        private readonly HostingEnvironment _hostingEnvironment;
        [BindProperty]
        public ProductsViewModel ProductsVM { get; set; }

        // using the constructor to allocate data to db object. 
        ...
        {
            var products = _db.Products.Include(m => m.ProductTypes).Include(m => m.Stores);
        ...
        {
            if (!ModelState.IsValid) // Add the products
            {
                return View(ProductsVM);
            }
            ...
            //Image being saved
            string webRootPath = _hostingEnvironment.WebRootPath; // retrieve the product from the database
            ...
            if (files.Count != 0)
            {
                ...
                {                                                               // change the product name to the product ID
            ...
            else // the default image if an image is not assigned, if user does not upload
            {
        ...
        public async Task<IActionResult> Edit(int id)
        {
            if (ModelState.IsValid)
            {
                ...
                productFromDb.StoreId = ProductsVM.Products.StoreId;
                await _db.SaveChangesAsync();
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            ProductsVM.Products = await _db.Products.Include(m => m.Stores).Include(m => m.ProductTypes).SingleOrDefaultAsync(m => m.Id == id);
